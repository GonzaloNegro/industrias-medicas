...
<script type="text/javascript">
function cargado(){
    swal.fire(  {title: "Se ha cargado correctamente su recibo.",
            icon: "success",
            showConfirmButton: true,
            showCancelButton: false,
            });
...
<script type="text/javascript">
function valorado(){
    swal.fire(  {title: "Se ha registrado su valoración.",
            icon: "success",
            showConfirmButton: true,
            showCancelButton: false,
            });
        ...
        <section class="ini">
            ...
            if($idRol == $rolgerente OR $idRol == $roladmin)
            {
                    echo "<table>
                            <thead>
                                <tr>
                                    <th><p style='text-align:right; margin-right: 5px;'>N°RECIBO</p></th>
                                    <th><p>FECHA</p></th>
                        ...
                        if(isset($_POST['btn2']))
                                {
                                    ...
                                    WHERE m.idEstadoDocumento = 10 AND (m.idDocumento LIKE '%$doc%' OR m.fecha LIKE '%$doc%' OR u.usuario LIKE '%$doc%' OR da.medico LIKE '%$doc%' OR da.paciente LIKE '%$doc%')
                                    ...
                                    while($listar = mysqli_fetch_array($consulta))
                                    {
                                        ...
                                        " 
                                            <tr>
                                                ...
                                                <td class='text-center text-nowrap'><a href=./subirRecibo.php?no=".$listar['idDocumento']." class='btn btn-sm btn-outline-danger'><i class='fa-solid fa-cloud-arrow-up'></i></a></td>
                                            </tr>
                                      ";
                                    } 
                                }
                            ...
                            if(isset($_POST['btn2']))
                                    {
                                        ...
                                        LEFT JOIN datosdocumento AS da ON da.idDocumento = d.idDocumento
                                        LEFT JOIN usuario AS u ON u.idUsuario = da.idUsuario
                                        WHERE m.idEstadoDocumento = 10 AND (m.idDocumento LIKE '%$doc%' OR m.fecha LIKE '%$doc%' OR u.usuario LIKE '%$doc%' OR da.medico LIKE '%$doc%' OR da.paciente LIKE '%$doc%') AND da.idUsuario = '$idUsu'
                                        GROUP BY d.idDocumento
                                        ORDER BY d.idDocumento DESC");
                                        while($listar = mysqli_fetch_array($consulta))
                                        {
                                        ...
                                        LEFT JOIN datosdocumento AS da ON da.idDocumento = d.idDocumento
                                        LEFT JOIN usuario AS u ON u.idUsuario = da.idUsuario
                                        WHERE m.idEstadoDocumento = 10 AND da.idUsuario = '$idUsu'
                                        GROUP BY d.idDocumento
                                        ORDER BY d.idDocumento DESC");
                                        while($listar = mysqli_fetch_array($consulta)) 
                                        {
