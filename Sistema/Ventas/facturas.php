...
<script type="text/javascript">
			function ok(){
				swal.fire(  {title: "Remito aceptado correctamente",
						icon: "success",
...
<script type="text/javascript">
function cargado(){
    swal.fire(  {title: "Se ha cargado correctamente su factura.",
            icon: "success",
            showConfirmButton: true,
            showCancelButton: false,
            });
        ...
        <section class="ini">
            ...
            if($idRol == $rolgerente OR $idRol == $roladmin)
            {
                    echo "<table>
                            <thead>
                                <tr>
                                    <th><p style='text-align:right; margin-right: 5px;'>N°FACTURA</p></th>
                                    <th><p>FECHA</p></th>
                        ...
                        if(isset($_POST['btn2']))
                                {
                                    ...
                                    WHERE m.idEstadoDocumento = 5 AND (m.idDocumento LIKE '%$doc%' OR m.fecha LIKE '%$doc%' OR u.usuario LIKE '%$doc%' OR da.medico LIKE '%$doc%' OR da.paciente LIKE '%$doc%')
                                    ...
                                    while($listar = mysqli_fetch_array($consulta))
                                    {
                                        ...
                                        " 
                                            <tr>
                                                ...
                                                <td class='text-center text-nowrap'><a href=./subirFactura.php?no=".$listar['idDocumento']." class='btn btn-sm btn-outline-danger'><i class='fa-solid fa-cloud-arrow-up'></i></a></td>
                                            </tr>
                                      ";
                                    } 
                                }
                            ...
                            if(isset($_POST['btn2']))
                                    {
                                        ...
                                        LEFT JOIN datosdocumento AS da ON da.idDocumento = d.idDocumento
                                        LEFT JOIN usuario AS u ON u.idUsuario = da.idUsuario
                                        WHERE m.idEstadoDocumento = 5 AND (m.idDocumento LIKE '%$doc%' OR m.fecha LIKE '%$doc%' OR u.usuario LIKE '%$doc%' OR da.medico LIKE '%$doc%' OR da.paciente LIKE '%$doc%') AND da.idUsuario = '$idUsu'
                                        GROUP BY d.idDocumento
                                        ORDER BY d.idDocumento DESC");
                                        while($listar = mysqli_fetch_array($consulta))
                                        {
                                            ...
                                            " 
                                                <tr>
                                                    ...
                                                    <td><h4 style='font-size:16px;text-transform:uppercase;text-align: left; margin-left: 5px;'>".$listar['medico']."</h4 ></td>
                                                    <td><h4 style='font-size:16px;text-transform:uppercase;text-align: left; margin-left: 5px;'>".$listar['paciente']."</h4 ></td>
                                                    <td class='text-center text-nowrap'> <a class='btn btn-sm btn-outline-danger' href=./confirmarFactura.php?no=".$listar['idDocumento']." target=new><svg xmlns='http://www.w3.org/2000/svg' width='20' height='20' fill='currentColor' class='bi bi-filetype-pdf' viewBox='0 0 16 16'>
                                        ...
                                        LEFT JOIN datosdocumento AS da ON da.idDocumento = d.idDocumento
                                        LEFT JOIN usuario AS u ON u.idUsuario = da.idUsuario
                                        WHERE m.idEstadoDocumento = 5 AND da.idUsuario = '$idUsu'
                                        GROUP BY d.idDocumento
                                        ORDER BY d.idDocumento DESC");
                                        while($listar = mysqli_fetch_array($consulta)) 
                                        {
