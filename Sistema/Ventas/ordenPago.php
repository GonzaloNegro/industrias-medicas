...
<script type="text/javascript">
function cargado(){
    swal.fire(  {title: "Se ha cargado correctamente su orden de pago.",
            icon: "success",
            showConfirmButton: true,
            showCancelButton: false,
            });
        ...
        <section class="ini">
            <div class="ini-tit">
                ...
                <?php }else{?>
                    <h1>COMPRAS</h1>
                <?php }?>
                <h5>ORDEN DE PAGO DE CLIENTES</h5>
            </div>
            <div class="container">

                <form method="POST" action="./ordenPago.php">
            ...
            if($idRol == $rolgerente OR $idRol == $roladmin)
            {
                    echo "<table>
                            <thead>
                                <tr>
                                    <th><p style='text-align:right; margin-right: 5px;'>N°OP</p></th>
                                    <th><p>FECHA</p></th>
                        ...
                        if(isset($_POST['btn2']))
                                {
                                    ...
                                    WHERE m.idEstadoDocumento = 6 AND (m.idDocumento LIKE '%$doc%' OR m.fecha LIKE '%$doc%' OR u.usuario LIKE '%$doc%' OR da.medico LIKE '%$doc%' OR da.paciente LIKE '%$doc%')
                                    ...
                                    while($listar = mysqli_fetch_array($consulta))
                                    {
                                        ...
                                        " 
                                            ...
                                            <td class='text-center text-nowrap'> <a class='btn btn-sm btn-outline-danger' href=./confirmarOP.php?no=".$listar['idDocumento']." target=new><svg xmlns='http://www.w3.org/2000/svg' width='20' height='20' fill='currentColor' class='bi bi-filetype-pdf' viewBox='0 0 16 16'>
                            ...
                            if(isset($_POST['btn2']))
                                    {
                                        ...
                                        LEFT JOIN datosdocumento AS da ON da.idDocumento = d.idDocumento
                                        LEFT JOIN usuario AS u ON u.idUsuario = da.idUsuario
                                        WHERE m.idEstadoDocumento = 6 AND (m.idDocumento LIKE '%$doc%' OR m.fecha LIKE '%$doc%' OR u.usuario LIKE '%$doc%' OR da.medico LIKE '%$doc%' OR da.paciente LIKE '%$doc%') AND da.idUsuario = '$idUsu'
                                        GROUP BY d.idDocumento
                                        ORDER BY d.idDocumento DESC");
                                        while($listar = mysqli_fetch_array($consulta))
                                        {
                                            ...
                                            " 
                                                ...
                                                <td><h4 style='font-size:16px;text-transform:uppercase;text-align: left; margin-left: 5px;'>".$listar['medico']."</h4 ></td>
                                                <td><h4 style='font-size:16px;text-transform:uppercase;text-align: left; margin-left: 5px;'>".$listar['paciente']."</h4 ></td>
                                                <td class='text-center text-nowrap'><a href=./subirOrdenPago.php?no=".$listar['idDocumento']." class='btn btn-sm btn-outline-danger'><i class='fa-solid fa-cloud-arrow-up'></i></a></td>
                                              </tr>
                                          ";
                                        } 
                                    }
                                        ...
                                        LEFT JOIN datosdocumento AS da ON da.idDocumento = d.idDocumento
                                        LEFT JOIN usuario AS u ON u.idUsuario = da.idUsuario
                                        WHERE m.idEstadoDocumento = 6 AND da.idUsuario = '$idUsu'
                                        GROUP BY d.idDocumento
                                        ORDER BY d.idDocumento DESC");
                                        while($listar = mysqli_fetch_array($consulta)) 
                                        {
