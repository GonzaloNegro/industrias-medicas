...
<style type="text/css" media="print">
    ...
    <section class="ini">
        <div class="ini-tit" id="titulo">
            <h1>Proveedores más recurrentes</h1>
        </div>
        ...
        <div class="contFilter" id="formulario">
                ...
                <div class="contFilter--order">
                        ...
                        if ($_POST["orden"] == '2'){echo 'Ordenar por Cantidad ventas';}
                        if ($_POST["orden"] == '3'){echo 'Ordenar por Monto';} 
                        ?>
                        </option>
                        <?php } ?>
                        <option value="">Sin orden</option>
                        <option value="1">Ordenar por nombre</option>
                        <option value="2">Ordenar por Cantidad de ventas</option>
                        <option value="3">Ordenar por Monto</option>
                </select>
                </div>
...
<!--             <h4 class="card-title">Filtro de búsqueda</h4>
        ...
        if ($_POST["buscar"] == '' AND $_POST['idGrupoProducto'] == '' AND $_POST['buscadesde'] == '' AND $_POST['buscahasta'] == '' AND $_POST['grupo'] == ''){ 
                $query ="SELECT COUNT(l.idLicitacion) AS TOTAL, u.usuario, SUM(da.monto) AS monto
                ...
                if ($_POST["buscar"] != '' ){ 
                        $query .= " WHERE (u.usuario LIKE LOWER('%".$aKeyword[0]."%')) AND l.idEstadoLicitacion = 6 GROUP BY u.usuario ";
                
                    for($i = 1; $i < count($aKeyword); $i++) {
                    if(!empty($aKeyword[$i])) {
                        $query .= " OR u.usuario LIKE '%" . $aKeyword[$i] . "%' WHERE l.idEstadoLicitacion = 6  ";
                    }
                    }
...
/*         $consulta=mysqli_query($datos_base, $query); */
        ...
        <div class="table-responsive" id="imprimirEsto">
            <table class="table">
                <thead>
                        <tr>
                            <th style="text-align: left;margin-left:5px; width:400px;">PROVEEDOR</th>
                            <th style="text-align: right;margin-right:5px; width: 300px;">COMPRAS CONCRETADAS</th>
                ...
                <?php While($rowSql = $sql->fetch_assoc()) {?>
                        <tr>
                        <td><h4 style="font-size:14px; text-align:left; margin-left: 5px;text-transform:uppercase;"><?php echo $rowSql["usuario"]; ?></h4></td>
                        <td><h4 style="font-size:14px; text-align: right; margin-right: 5px;"><?php echo $rowSql["TOTAL"]; ?></h4></td>
                        <td><h4 style="font-size:14px; text-align: right; margin-right: 5px;"><?php echo "$".number_format($rowSql["monto"],2, ',','.'); ?></h4></td>
                        </tr>
        ...
        <form id="formu" action="../Exportar/ExcelProvRecurrentes.php" method="POST">
