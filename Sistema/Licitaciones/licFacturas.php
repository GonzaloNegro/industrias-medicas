...
<script type="text/javascript">
			function ok(){
				swal.fire(  {title: "Se ha aceptado el remito correctamente.",
						icon: "success",
...
<?php include('../Layouts/licitacionesHeader.php'); ?>
    <main>
        <section class="ini">
            <div class="ini-tit">
                <h1>LICITACIONES</h1>
                <h5>FACTURAS DE LOS PROVEEDORES</h5>
            </div>
            <div class="container">

                <form method="POST" action="./licFacturas.php">
                        ...
                        if(isset($_POST['btn2']))
                                {
                                    ...
                                    WHERE m.idEstadoLicitacion = 4 AND (l.idLicitacion LIKE '%$doc%' OR m.fecha LIKE '%$doc%' OR u.usuario LIKE '%$doc%')
                                    ...
                                    while($listar = mysqli_fetch_array($consulta)) 
                                    {
                                        ...
                                        " 
                                            <tr>
                                                ...
                                                <td class='text-center text-nowrap'> <a class='btn btn-sm btn-outline-danger' href=./confirmarLicFactura.php?no=".$listar['idLicitacion']." target=new><svg xmlns='http://www.w3.org/2000/svg' width='20' height='20' fill='currentColor' class='bi bi-filetype-pdf' viewBox='0 0 16 16'>
                            ...
                            if(isset($_POST['btn2']))
                                    {
                                        ...
                                        LEFT JOIN datoslicitacion AS da ON da.idLicitacion = l.idLicitacion
                                        LEFT JOIN usuario AS u ON u.idUsuario = da.idUsuario
                                        WHERE m.idEstadoLicitacion = 4 AND (l.idLicitacion LIKE '%$doc%' OR m.fecha LIKE '%$doc%' OR u.usuario LIKE '%$doc%') AND da.idUsuario = '$idUsu'
                                        GROUP BY l.idLicitacion
                                        ORDER BY l.idLicitacion DESC");
                                        while($listar = mysqli_fetch_array($consulta)) 
                                        {
                                            ...
                                            " 
                                                <tr>
                                                    ...
                                                    <td><h4 style='font-size:16px;'>".$fec."</h4 ></td>
                                                    <td><h4 style='font-size:16px;text-transform:uppercase;text-align: left; margin-left: 5px;'>".$listar['usuario']."</h4 ></td>
                                                    <td class='text-center text-nowrap'><a href=./subirLicFactura.php?no=".$listar['idLicitacion']." class='btn btn-sm btn-outline-danger'><i class='fa-solid fa-cloud-arrow-up'></i></a></td>
                                                </tr>
                                            ";
                                        } 
                                    }
                                        ...
                                        LEFT JOIN datoslicitacion AS da ON da.idLicitacion = l.idLicitacion
                                        LEFT JOIN usuario AS u ON u.idUsuario = da.idUsuario
                                        WHERE m.idEstadoLicitacion = 4 AND da.idUsuario = '$idUsu'
                                        GROUP BY l.idLicitacion
                                        ORDER BY l.idLicitacion DESC");
                                        while($listar = mysqli_fetch_array($consulta)) 
                                        {
